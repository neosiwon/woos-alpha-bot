#!/bin/bash
# Patch v2: 3차 알람 포맷 + MONITOR 채널 (2차+3차+일일리포트) + GitHub 자동 push
# 사용법: VM의 ~/woos-alpha-bot/ 에서 실행
#
# ⚠️ 실행 전 ~/woos-alpha-bot/.env 에 아래 한 줄 필요:
...
if [ -f .env ] && ! grep -q "TELEGRAM_CHAT_ID_MONITOR" .env; then
  echo "⚠️  .env 에 TELEGRAM_CHAT_ID_MONITOR=-5197822664 추가하고 다시 실행하세요."
  read -p "지금 자동 추가할까요? [y/N] " yn
  if [[ "$yn" =~ ^[Yy]$ ]]; then
...
cp bot/report.js bot/report.js.bak_$(date +%s)
...
echo "=== Write report.js ==="
cat > bot/report.js << 'FILE_EOF_REPORT'
...
async function _send(text) {
  // 9시 리포트: MONITOR + (옵션) PRIVATE 둘 다 발송
  const token = cfg.TELEGRAM_BOT_TOKEN;
  if (!token) { console.log('[report] no token\n' + text); return; }
  const monitor = cfg.TELEGRAM_CHAT_ID_MONITOR;
  const priv = cfg.TELEGRAM_CHAT_ID_PRIVATE || cfg.TELEGRAM_CHAT_ID;
  const targets = [];
  if (monitor) targets.push(monitor);
  if (priv && priv !== monitor) targets.push(priv);
  if (!targets.length) { console.log('[report] no chat ids\n' + text); return; }
  for (const chatId of targets) {
    try {
      ...
      if (!r.ok) {
        ...
        console.error(`[report] HTTP ${r.status} to ${chatId}: ${body}`);
      } else {
        console.log(`[report] sent to ${chatId}`);
      }
    } catch (e) {
      console.error(`[report] send fail ${chatId}: ${e.message}`);
    }
  }
}

// Git: reports/YYYY-MM-DD.md 저장 + commit + push
function _commitReport(dateStr, text) {
  try {
    const path = require('path');
    const { execSync } = require('child_process');
    const reportDir = path.join(process.cwd(), 'reports');
    if (!fs.existsSync(reportDir)) fs.mkdirSync(reportDir, { recursive: true });
    const filepath = path.join(reportDir, `${dateStr}.md`);
    fs.writeFileSync(filepath, text + '\n');
    execSync(`git add reports/${dateStr}.md`, { stdio: 'pipe' });
    try {
      execSync(`git commit -m "report: ${dateStr}"`, { stdio: 'pipe' });
    } catch (e) {
      // nothing to commit 인 경우
      console.log('[report] nothing to commit');
      return;
    }
    execSync('git push', { stdio: 'pipe' });
    console.log(`[report] git pushed: reports/${dateStr}.md`);
  } catch (e) {
    console.error(`[report] git fail: ${e.message}`);
  }
}
...
mkdir -p reports
[ -f reports/.gitkeep ] || touch reports/.gitkeep

echo "=== Syntax check ==="
node --check bot/notify.js && echo "notify OK"
node --check bot/report.js && echo "report OK"
node --check config/config.js && echo "config OK"

echo "=== Git push ==="
git add bot/notify.js bot/report.js config/config.js reports/.gitkeep
git commit -m "patch v2: alarm format, MONITOR channel, daily report auto-push" || echo "(nothing to commit)"
...
echo "=== DONE ==="
echo "채널 라우팅:"
echo "  - MONITOR (-5197822664): 2차 매집 + 3차 매수 + 9시 일일리포트"
echo "  - GROUP   (기존 단톡):   3차 매수만"
echo "  - PRIVATE (옵션):         9시 일일리포트 (있을 때만)"
echo ""
echo "9시 리포트는 텔레그램 발송 + ~/woos-alpha-bot/reports/YYYY-MM-DD.md 로 git auto-push"
