...
function _ensureHeader() {
  if (!fs.existsSync(cfg.VERIFY_LOG_FILE)) {
    fs.writeFileSync(cfg.VERIFY_LOG_FILE, '\uFEFF신호시각,종목,진입가,MFE%,MAE%,종료가,종료수익%,5%달성,국면,진입체결강도\n');
  }
}
...
function _finalize(t) {
  ...
  const endPrice = t.lastPrice || t.entryPrice;
  const endPct = ((endPrice - t.entryPrice) / t.entryPrice * 100).toFixed(2);
  const hit5 = (t.mfe - t.entryPrice) / t.entryPrice >= 0.05 ? 'O' : 'X';
  const row = [t.signalTime, t.symbol, t.entryPrice, mfePct, maePct, endPrice, endPct, hit5, t.regime, t.execStrength.toFixed(1)].join(',') + '\n';
