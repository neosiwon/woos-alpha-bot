# woos_signal_fsm.py  (v5 — 보수적 저항기반 TP: 진입가 위 저항을 TP, 없으면 고정, 손절 -4%)
...
# ---- 신호 양식 ----
def resist_tp(sym, entry_px):
    # 보수적 TP/손절: 진입가 위 실제 저항(단기고·매물대상단·공급OB)을 TP로.
    #   - 저항 있으면 가까운 순 TP1·TP2·TP3 (TP1은 최소 +5%)
    #   - 저항 없으면 (이미 돌파, MOVE형) 보수적 고정 +5/+10/+15
    #   - 손절은 -4% 고정 (지지선은 참고 표시만, 실제 지지는 -8~20%로 너무 멂)
    # 반환: dict(tp=[(라벨,가격,%)], box_top_pct, zone, cycle, support, has_resist)
    ...
    if not r or r["top"] <= r["bot"]:
        return None
    cur = entry_px
    zone, cyc = r["zone"], r["cycle"]
    sh = r["swing_hi"][1] if r["swing_hi"] else None
    sl = r["swing_lo"][1] if r["swing_lo"] else None
    top, bot = r["top"], r["bot"]
    supply = [ob[2] for ob in r["order_blocks"] if ob[1] == "supply"]
    demand = [ob[3] for ob in r["order_blocks"] if ob[1] == "demand"]
    # 진입가 위 저항 (TP 후보) — 2%+ 위, 가까운 순, 중복 근사 제거
    raw = sorted(v for v in ([sh, top] + supply) if v and v > cur * 1.02)
    resist = []
    for v in raw:
        if not resist or v > resist[-1] * 1.03:     # 3% 이상 벌어진 것만 (중복 제거)
            resist.append(v)
    # 진입가 아래 지지 (참고용 표시)
    support = sorted((v for v in ([sl, bot] + demand) if v and v < cur * 0.99), reverse=True)
    sup_px = support[0] if support else None
    box_top_pct = (top / cur - 1) * 100
    # TP 구성
    tp = []
    if resist:
        # 저항 기반: TP1은 첫 저항(단 최소 +5%), 이후 저항들
        levels = []
        for v in resist[:3]:
            levels.append(v)
        # TP1 = 첫 저항이 +5% 미만이면 +5%로 올림
        tp1_px = max(levels[0], cur * 1.05)
        tp.append(("TP1", tp1_px, (tp1_px/cur - 1)*100))
        if len(levels) >= 2:
            tp.append(("TP2", levels[1], (levels[1]/cur - 1)*100))
        elif top > tp1_px * 1.02:
            tp.append(("TP2", top, box_top_pct))
        if len(levels) >= 3:
            tp.append(("TP3", levels[2], (levels[2]/cur - 1)*100))
        elif len(tp) >= 2 and top > tp[-1][1] * 1.02:
            tp.append(("TP3", top, box_top_pct))
    else:
        # 저항 없음 (이미 돌파) → 보수적 고정
        tp = [("TP1", cur*1.05, 5.0), ("TP2", cur*1.10, 10.0), ("TP3", cur*1.15, 15.0)]
    return dict(tp=tp, box_top_pct=box_top_pct, zone=zone, cycle=cyc,
                support=sup_px, has_resist=bool(resist))
...
def build_signal(exch, sym, names, bs, grip, cusum, vwap, last_px, gap_min, seq, ftp=None):
    ...
    if ftp:
        if ftp["has_resist"]:
            ...
            lines.append(f"- 위쪽 저항 없음 (보수적 TP)")
    ...
    if ftp and ftp.get("support"):
        lines.append(f"  (지지 참고 {ftp['support']:.4g}원)")
    # 저항 기반 TP (없으면 고정 fallback)
    ...
    for key, p, bs in cands:
        ...
        if r["state"] in ("ACCUM", "PRELAUNCH") and grip is not None and grip >= GRIP_MIN \
           and band <= ENTRY_BAND_PCT and r["notified_seq"] != r["seq"]:
            ftp = resist_tp(sym, last_px) if exch == "BT" else None
