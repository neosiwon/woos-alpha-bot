#!/usr/bin/env python3
"""
alpha_scanner_v0_4.py — (A형 장악률 복원: 봇net/유통가치)
...
# ===== Bithumb internal circulating supply (grip-rate denominator) =====
#   A-type grip rate = bot_net(KRW) / (supply * price) — force control vs tradable supply.
#   GWEI 14.7% / SLX 15.7% / MOVE 22.6% = launch ; ONDO 0.4% / NEAR 0.4% = no launch.
#   Bithumb discloses internal supply (accumulationDepositAmt); Upbit does not.
_SUPPLY_CACHE = {}
_COINTYPE = {}
def _load_cointypes():
    global _COINTYPE
...
def get_supply(sym):
    """Bithumb internal circulating supply (count). Cached per run."""
...
GRIP_MIN        = 10.0         # A-type grip rate %: bot_net / (supply*price)
                               # verified launch 14.7~22.6% vs non-launch 0.4% (cut at 10%)
...
def run(date_str=None):
    ...
    if not rows:
        return []
    # B-type: volume CUSUM as market-wide MAD outlier
    zs = mad_scores([m["cusum_peak"] for _, m in rows])
    for i, (s, m) in enumerate(rows):
        m["cusum_mad"] = zs[i]
    # A-type: GRIP RATE = bot_net(KRW) / (bithumb supply * price)
    #   verified: launch 14.7~22.6% vs non-launch 0.4% (35x gap, no overlap)
    for s, m in rows:
        supply = get_supply(s)
        if supply and m["last_px"]:
            supply_val = supply * m["last_px"]          # circulating supply value (KRW)
            m["grip"] = m["bot_net"] / supply_val * 100 if supply_val > 0 else 0.0
        else:
            m["grip"] = None                            # Upbit-only / no supply data
    out = []
    for s, m in rows:
        A = m["grip"] is not None and m["grip"] >= GRIP_MIN
        B = m["cusum_mad"] >= CUSUM_MAD_MIN
        if A or B:
            out.append((s, "A+B" if A and B else ("A" if A else "B"), m))
    # sort: grip + cusum (both signals)
    out.sort(key=lambda x: -((x[2].get("grip") or 0) + x[2]["cusum_mad"]))
    return out

def report(results, date_str):
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"\n=== alpha_scanner {date_str} @ {ts} : {len(results)} signals ===")
    print(f"{'SYM':9s}{'TAG':5s}{'grip%':>7s}{'botQ(M)':>8s}{'botA(M)':>8s}{'cusumMAD':>9s}{'vol억':>7s}{'chg%':>6s}")
    for s, tag, m in results[:TOP_N]:
        g = f"{m['grip']:.1f}" if m.get('grip') is not None else "-"
        print(f"{s:9s}{tag:5s}{g:>7s}{m['bot_qty']/1e6:>8.0f}{m['bot_amt']/1e6:>8.0f}{m['cusum_mad']:>9.1f}{m['daily_vol']/1e8:>7.0f}{m['chg_pct']:>+6.0f}")
    # save json
    try:
        payload = {"ts": ts, "date": date_str,
                   "signals": [{"sym": s, "tag": t, "bot_net": m["bot_net"],
                                "grip": (round(m["grip"],2) if m.get("grip") is not None else None),
