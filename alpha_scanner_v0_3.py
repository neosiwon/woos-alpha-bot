#!/usr/bin/env python3
"""
alpha_scanner_v0_3.py — (봇 두갈래 + 봇net 전체시장 MAD이상치)
...
BOT_MAD_MIN     = 3.5          # A-type: bot net as market-wide MAD outlier (not abs threshold)
...
def run(date_str=None):
    ...
    for i, (s, m) in enumerate(rows):
        m["cusum_mad"] = zs[i]
    # A-type: bot net accumulation as market-wide MAD outlier (same philosophy)
    #   GWEI 585M is a clear outlier vs market; AUCTION 33M is ordinary.
    bz = mad_scores([m["bot_net"] for _, m in rows])
    for i, (s, m) in enumerate(rows):
        m["bot_mad"] = bz[i]
    out = []
    for s, m in rows:
        A = m["bot_mad"] >= BOT_MAD_MIN
        B = m["cusum_mad"] >= CUSUM_MAD_MIN
        if A or B:
            out.append((s, "A+B" if A and B else ("A" if A else "B"), m))
    out.sort(key=lambda x: -(x[2]["cusum_mad"] + x[2]["bot_mad"]))
    return out

def report(results, date_str):
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"\n=== alpha_scanner {date_str} @ {ts} : {len(results)} signals ===")
    print(f"{'SYM':9s}{'TAG':5s}{'botQ(M)':>8s}{'botA(M)':>8s}{'botMAD':>7s}{'cusumMAD':>9s}{'vol억':>7s}{'chg%':>6s}")
    for s, tag, m in results[:TOP_N]:
        print(f"{s:9s}{tag:5s}{m['bot_qty']/1e6:>8.0f}{m['bot_amt']/1e6:>8.0f}{m['bot_mad']:>7.1f}{m['cusum_mad']:>9.1f}{m['daily_vol']/1e8:>7.0f}{m['chg_pct']:>+6.0f}")
    # save json
    try:
        payload = {"ts": ts, "date": date_str,
                   "signals": [{"sym": s, "tag": t, "bot_net": m["bot_net"],
                                "bot_mad": round(m["bot_mad"],2),
